...
class user_checker extends uvm_component;
	...
	function shortreal get_item_price(bit[4:0] item_num, bit [8:0] client_id);
        ...
        `uvm_info(get_type_name(), $sformatf("Base price: %0f, item discount: %0f", price, item_discount), UVM_DEBUG)
		// Расчет скидки
	...
	function user_transaction calculate_exp_transaction(
        ...
        `uvm_info(get_type_name(), $sformatf("Calculated balance: %0f, item price: %0f", balance, item_price), UVM_DEBUG)
